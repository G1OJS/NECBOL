...
model = NECModel(working_dir="nec_wkg",
                 model_name = "Dipole behind sheet",
                 nec_exe_path="C:\\tmp\\nec2dxs11k.exe")

model.set_frequency(MHz =  300)

model.set_angular_resolution(az_step_deg = 90, el_step_deg = 1)
...
diel_sheet=comps.thin_sheet(model,  epsillon_r = 400, length_m = .5,
                                    height_m = .5,
                                    dielectric_thickness_m = 0.1,
                                    grid_pitch_m = 0.025 )
